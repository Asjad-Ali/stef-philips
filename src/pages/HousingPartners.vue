<template>
  ...
  <div v-if="Object.keys(store.housing_partners).length">
    <div class="w-full min-h-[500px] mb-[50px]">
      <PageBanner :Banner="store.housing_partners" />
    </div>
    
    <div class="container py-[100px]">
      <SectionContentImage :Cards="store.housing_partners.housing_partner_topcards.data"/>
    </div>
    ...
    <div class="container pt-[50px] pb-[100px]">
      <div class="grid md:grid-cols-3 gap-[24px]">
        <Cards :Cards="store.housing_partners.housing_partner_2_crads.data" />
      </div>
    </div>
    ...
    <div class="bg-second min-h-[600px] text-white w-full">
      <div class="container py-[80px]">
        ...
        <div class="card generic-slider mt-[20px]">
          <Carousel
            :value="store.housing_partners.housing_partner_bluecards.data"
            :numVisible="2"
            :numScroll="2"
          ...
          <template #item="Card">
              <div class="m-2 px-[30px] pt-[40px] pb-3 lg:h-[380px] bg-white">
                <div class="text-[#20407C]">
                  <img class="w-[55px] pb-5" :src="Card.data.attributes.icon.data.attributes.url" alt />
                  <h4 class="mb-3 font-black-custom text-[18px]">
                    {{ Card.data.attributes.title }}
                  </h4>
                  <p class="mt-0 mb-3 text-[16px]">
                    {{ Card.data.attributes.description }}
                  </p>
                </div>
              </div>
            </template>
    ...
    <div class="container py-[100px]">
      <SectionContentImage v-if="Object.keys(store.housing_partners).length" :Cards="store.housing_partners.housing_partner_cardbottoms.data"/>
    </div>
    ...
    <PartnerUs :Cards="store.housing_partners.housing_partner_bluecardlasts.data" />
...
onMounted(() => {  
  if(!Object.keys(store.housing_partners).length)
    store.housingPartners()
